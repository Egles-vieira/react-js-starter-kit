...
module.exports = {
  async findAll() {
    const result = await db.query(`
      SELECT 
        nf.*,
        -- usa o campo existente no banco:
        nf.transportadora_id AS transportadora_id,    -- BIGINT atual
        c.id   AS cliente_id,
        c.nome AS cliente_nome,
        c.cod_cliente AS cliente_cod_cliente,
        ee.endereco_completo AS endereco_entrega_completo
      FROM public.notas_fiscais nf
      LEFT JOIN public.clientes c ON nf.cliente_id = c.id
      -- (sem join de transportadoras por incompatibilidade BIGINT ↔ UUID)
      LEFT JOIN public.endereco_entrega ee ON nf.endereco_entrega_id = ee.id
      ORDER BY nf.id DESC
    `);
    return result.rows;
},
